...
**왜 이게 필요하냐면** — 컨테이너는 **자주 지우고 다시 만드는 게 정상**이기 때문입니다. 버전을 올리거나 설정을 바꾸면 새로 띄우거든요. 그때마다 **데이터베이스 내용이나 업로드한 파일이 날아가면 안 되니까**, 데이터는 컨테이너 **밖에** 둡니다.
...
> **바인드 마운트와 뭐가 다른가** 물으면 → 볼륨은 **도커가 관리하는 창고**(운영 데이터용), 바인드 마운트는 **내 컴퓨터 폴더를 직접 연결**(개발용). → **17번**에 정리해뒀습니다.
...
**📖 먼저 용어부터 — 포트는 "문 번호", 볼륨은 "밖에 둔 창고"**

🗣️ **"재현성 얘기를 드리기 전에, 포트와 볼륨이 뭔지부터 짚고 가겠습니다."**

| | 포트 | 볼륨 |
|---|---|---|
| **정의** | 컨테이너 **안팎을 잇는 문 번호** | 컨테이너 **밖에 따로 둔 저장 공간** |
| **비유** | 강 건너 섬에 놓는 **다리** | 호텔 방 밖에 빌린 **사물함** |
| **없으면** | 밖에서 컨테이너에 **못 닿음** | 컨테이너 지우면 데이터 **같이 사라짐** |
| **설정** | `-p 8080:80` (호스트:컨테이너) | `-v my-data:/data` (볼륨명:컨테이너경로) |
| **재현할 때 문제** | 남의 컴퓨터에서 그 번호가 **이미 쓰이면 충돌** | 어떤 볼륨을 어디에 붙였는지 **모르면 재현 불가** |

**그래서 이 문항이 왜 나오냐면**

```
docker build  →  이미지 완성          ← 웹서버·HTML 은 여기 담김
docker run -p 8080:80 -v my-data:/data  ← 포트·볼륨은 '여기서' 정함
```

포트와 볼륨은 **이미지에 담기지 않습니다.** 실행하는 순간에 정하는 값이에요.

그래서 평가자가 clone 해서 `build` 하면 **이미지는 똑같이 나오지만**, **"몇 번 포트로 띄우라는 거지?"** 는 이미지가 알려주지 않습니다.

> 🔌 가전제품 같은 겁니다. 제품은 똑같이 받았는데 **어느 콘센트에 꽂을지**는 제품에 안 적혀 있죠. **설명서에 적어줘야** 합니다.

🗣️ **"그래서 이 문항은 '적어뒀냐'를 묻는 겁니다."**

---

**① 네. "남이 따라 할 때 깨지는 곳"이 어디인지 먼저 따져서, 그 세 군데를 막았습니다.**

🗣️ **"재현이 안 되는 이유는 뻔합니다. 포트가 겹치거나, 설정값이 다르거나, 볼륨을 어디에 붙였는지 모르거나. 이 세 개를 각각 막았습니다."**

| 깨지는 원인 | 제가 한 조치 |
|---|---|
| **① 포트가 겹친다**<br>남의 컴퓨터에 이미 8080이 쓰이는 중일 수 있음 | 포트를 **용도별로 미리 갈라두고**, 겹치면 **왼쪽 숫자만 바꾸라고** README에 적어둠 |
| **② 설정을 바꾸기 어렵다**<br>포트를 바꾸려면 이미지를 다시 만들어야 함 | 설정을 이미지에 **박지 않고 밖에서 넣게** 만듦 → **명령 한 줄**로 변경 |
| **③ 볼륨을 뭘 어디에 붙였는지 모른다** | 볼륨 이름(`my-data`)과 연결 경로(`/data`)를 **재현 절차에 그대로** 적어둠 |
...
> 실제로 8080을 겹쳐 써서 `port is already allocated` 에러를 겪었습니다. 그래서 **처음부터 번호를 나눠 쓴 것**입니다.

---

**③ 설정 — 이미지에 박지 않고 밖에서 넣게 했습니다** ⭐ 여기가 핵심

nginx 설정 파일에 포트 번호를 **그냥 써두면**, 포트를 바꿀 때마다 파일을 고치고 **이미지를 다시 구워야** 합니다.

```
[박아둔 경우]        listen 80;
                    → 3000으로 바꾸려면?  파일 수정 + docker build 다시  😩

[빈칸으로 둔 경우]    listen ${APP_PORT};
                    → 3000으로 바꾸려면?  docker run -e APP_PORT=3000   😊
```

🗣️ **"설정을 코드 안에 박아두면 못 바꿉니다. 그래서 빈칸으로 두고, 실행할 때 값을 넣게 했습니다. 이미지는 그대로 두고 명령만 바꾸면 됩니다."**
